...
OUTPUT_NODE = 10            #输入层的节点数。这个等于类别的数目。因为在MNIST数据集中
                            #需要区分的是0~9这10个数字，所以这里输出层的节点数为10

#配置神经网络的参数。
LAYER1_NODE = 500           #隐藏层节点数。这里使用只有一个隐藏层的网络结构作为样例。
                            #这个隐藏层有500个节点。
BATCH_SIZE = 100            #一个训练batch中训练数据个数。数字越小时，训练过程越接近
...
#一个辅助函数，给定神经网络中的输入和所有参数，计算神经网络中的前向传播结果。在这里
#定义了一个舒勇ReLU激活函数的三层全连接神经网络。通过加入隐藏层实现了多层网络结构，
#提高RelU激活函数实现了去线性化。在这个函数中也支持用于计算参数平均值的类，
#这样方便在测试是使用滑动平均模型。
def inference(input_tensor, avg_class, weights1, biases1, weights2, biases2):
    ...
    if avg_class == None:
        ...
        #计算没有有影响，于是在计算整个神经网络的前向传播时可以不加入最后的softmax层。
...
def train(mnist):
    ...
    #只包含了0~9中的一个数字，所以可以使用这个函数来计算交叉熵的损失。这个函数的第一个
    ...
    #设置指数衰减的学习率。
    learning_rate = tf.train.exponential_decay(LEARNING_RATE_BASE,          #基础的学习率，随着迭代的进行，更新变量时使用的
                                                                            #学习率在这个基础上递减。
                                               global_step,         #当前迭代的轮数。
    ...
    with tf.Session() as sess:
        ...
        #准备测试数据。在真实的应用中，这部分数据在训练时是不可兼得，这个数据只是作为模型
        #优劣的最后评判标准。
        ...
        for i in range(TRAINING_STEPS):
            ...
            if i % 1000 == 0:
                ...
                #可以处理所有验证数据。为了计算方便，本样例程序没有验数据划分为更
