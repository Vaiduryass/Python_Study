...
#个张量名称就是'pool_3/_reshape:0'。在训练模型时，可以通过tensor.name来获取张
#量的名称。
...
#图片数据文件夹。在这个文件夹中每一个子文件代表一个需要区分的类别，每个子文件夹中
#存放了对应类别的图片。
...
def run_bottleneck_on_image(sess,image_data, image_data_tensor, bottleneck_tensor):
    ...
    #经过卷积神经网络处理的结果是一个四维的数组，需要将这个结果压缩成一个特征
    #向量（一维数组）。
...
def get_test_bottlenecks(sess, image_lists, n_classes, jpeg_data_tensor, bottleneck_tensor):
    bottlenecks = []
    ground_truths = []
    label_name_list = list(image_lists.ketys())
    #枚举所有的类别和每个类别中的测试图片。
